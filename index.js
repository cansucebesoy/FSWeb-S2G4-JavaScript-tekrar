...
ucebolunenlerintoplami = ucetambolunenler.reduce((total,current) => total + current, 0);
...
tekrarSayisi = {};
tekraredensayilar = sayilar.forEach(sayi => {
...
for (const sayi in tekrarSayisi) {
  const tekrarSayisi = tekrarSayisi[sayi];
  const string = `${sayi} sayısı ${tekrarSayisi} kere tekrar edilmiştir`;
  tekrarSayisi.push(string);
}
